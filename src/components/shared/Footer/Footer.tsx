...
export default function Footer({ links, footerNote, logo }: FooterProps) {
    return (
        <footer className="bg-white97 dark:bg-gray10 text-black dark:text-white px-4 md:px-8 lg-custom:!px-[120px] 2xl:!px-[162px] huge:px-0">
                        ...
                        {links?.map(({ title, to, items }, i) => (
                            <div key={i} className="flex flex-col space-y-2 pb-5 border-b odd:border-r border-r-white90 dark:border-r-gray15 border-white90 dark:border-gray15 lg-custom:pb-0 lg-custom:border-none">
                                <Link
                                    to={to}
                                    onClick={scrollToTop}
                                    className="font-meduim text-base 2xl:whitespace-nowrap md:text-[18px] 2xl:text-lg text-gray40 dark:text-gray60 hover:text-purple70 transition-colors mb-4">
                                    {title}
                                </Link>
                                {items.map(({ label, to }, j) => (
                                    <Link
                                        ...
                                        to={to}
                                        onClick={scrollToTop}
                                        className="font-meduim text-black dark:text-white 2xl:whitespace-nowrap hover:text-gray60 text-sm md:text-base 2xl:text-lg transition-colors leading-6">
                                        {label}
                                    </Link>
                                ))}
                            </div>
                ...
                <div className="mt-10 border-t border-gray30 pt-8">
                    <div className="flex flex-col-reverse gap-[20px] lg-custom:flex-row justify-between items-center">
                        ...
                        <div className="flex space-x-4 mt-4 lg:mt-0">
                            <a href="#" className="w-[52px] h-[52px] bg-white99 dark:bg-gray08 rounded-full flex items-center justify-center hover:bg-purple60 hover:-translate-y-1 duration-300 ease-in-out">
                                <FacebookIcon

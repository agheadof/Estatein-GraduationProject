...
const Navbar = ({ isBannerVisible }: NavbarProps) => {
  ...
  return (
    <nav
      className={`bg-white97 dark:bg-gray10 text-black dark:text-white py-5 px-4 lg-custom:px-10 fixed w-full z-50 border-b-2 border-b-white90 dark:border-b-gray15 transition-all duration-700 ease-in-out
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.ul initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="flex flex-col gap-4 mt-4 md:hidden bg-white99 dark:bg-gray08 p-5 rounded-lg border border-white90 dark:border-gray15 ">

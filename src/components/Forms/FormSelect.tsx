...
function FormSelect({ label, name, value, placeholder, onChange, options, error, classExtra = "bg-white97 dark:bg-gray10", classIcon, children }: SelectProps) {
  ...
  return (
    <div className="flex flex-col group shrink-0" ref={selectRef}>
      ...
      <div className={`relative bg-purple90 flex flex-col border-1 rounded-lg border-white90 dark:border-gray15 hover:text-purple75 transition-colors duration-300 ${classExtra}`}>
        <div className={`flex gap-2.5 px-4 py-5 h-[53.45px] items-center cursor-pointer`} onClick={() => setIsOpen(prev => !prev)}>
          {children}
      ...
      {isOpen && createPortal(
        <ul
          ...
          className="bg-white dark:bg-gray10 border border-gray-200 dark:border-gray15 rounded-lg shadow-lg"
        >
          {options.map(opt => (
            <li
              key={opt}
